...
has_next_parent_acond_parent_iterator (BtorPartialParentIterator * it)
{
  assert (it);
  return it->cur
    && BTOR_IS_ARRAY_COND_NODE (BTOR_REAL_ADDR_NODE (it->cur));
}
...
merge_simplified_exp_const (Btor * btor, BtorNode * a, BtorNode * b)
{
  ...
  if (rep_a == BTOR_INVERT_NODE (rep_b))
    return 0;
...
static BtorNode *
new_binary_exp_node (Btor * btor, BtorNodeKind kind, BtorNode * e0,
		     BtorNode * e1, int len)
{
  BtorBVNode *exp;
...
static BtorNode *
new_ternary_exp_node (Btor * btor, BtorNodeKind kind, BtorNode * e0,
		      BtorNode * e1, BtorNode * e2, int len)
{
  BtorBVNode *exp;
...
static BtorNode *
new_write_exp_node (Btor * btor, BtorNode * e_array, BtorNode * e_index,
		    BtorNode * e_value)
{
  BtorMemMgr *mm;
  BtorNode *exp;
...
static BtorNode *
new_acond_exp_node (Btor * btor, BtorNode * e_cond, BtorNode * a_if,
		    BtorNode * a_else)
{
  BtorMemMgr *mm;
  BtorNode *exp;
...
find_binary_exp (Btor * btor, BtorNodeKind kind, BtorNode * e0, BtorNode * e1)
{
  ...
  hash =
    (((unsigned int) BTOR_REAL_ADDR_NODE (e0)->id +
      (unsigned int) BTOR_REAL_ADDR_NODE (e1)->id) *
     BTOR_NODE_UNIQUE_TABLE_PRIME) & (btor->unique_table.size - 1);
  result = btor->unique_table.chains + hash;
  cur = *result;
  while (cur)
    {
 ...
 * found. */
static BtorNode **
find_ternary_exp (Btor * btor, BtorNodeKind kind, BtorNode * e0,
		  BtorNode * e1, BtorNode * e2)
{
  BtorNode *cur, **result;
  unsigned int hash;
  assert (btor);
  ...
  assert (e2);
  hash =
    (((unsigned) BTOR_REAL_ADDR_NODE (e0)->id +
...
btor_const_exp (Btor * btor, const char *bits)
{
  ...
  if (!*lookup)
    ...
    btor_log_2_util (btor->unique_table.size) < BTOR_NODE_UNIQUE_TABLE_LIMIT)
	{
...
static BtorNode *
binary_exp (Btor * btor, 
            BtorNodeKind kind, BtorNode * e0, BtorNode * e1, int len)
{
...
static BtorNode *
ternary_exp (Btor * btor, BtorNodeKind kind, BtorNode * e0,
	     BtorNode * e1, BtorNode * e2, int len)
{
  BtorNode **lookup;
  assert (btor);
...
dump_exps (Btor * btor, FILE * file, BtorNode ** roots, int nroots)
{
  ...
  for (i = 0; i < BTOR_COUNT_STACK (stack); i++)
	...
	case BTOR_SLICE_NODE:
	  fprintf (file,
		   "slice %d %d %d %d",
		   e->len, BTOR_GET_ID_NODE (e->e[0]), e->upper, e->lower);
	  break;
...
btor_print_stats_btor (Btor * btor)
{
  ...
  btor_msg_exp (btor, "variable substitutions: %d",
		     btor->stats.var_substitutions);
  btor_msg_exp (btor, "array substitutions: %d",
		     btor->stats.array_substitutions);
  btor_msg_exp (btor, "embedded constraint substitutions: %d",
  ...
  if (verbosity > 2)
    {
      ...
      btor_msg_exp (btor, "number of expressions ever created: %lld",
		    btor->stats.expressions);
  ...
  if (btor->stats.lod_refinements)
    {
      btor_msg_exp (btor, " array axiom 1 conflicts: %d",
			 btor->stats.array_axiom_1_conflicts);
      btor_msg_exp (btor, " array axiom 2 conflicts: %d",
			 btor->stats.array_axiom_2_conflicts);
      btor_msg_exp (btor, " average lemma size: %.1f",
			 BTOR_AVERAGE_UTIL (btor->stats.lemmas_size_sum,
					    btor->stats.lod_refinements));
      btor_msg_exp (btor, " average linking clause size: %.1f",
			 BTOR_AVERAGE_UTIL (btor->stats.lclause_size_sum,
					    btor->stats.lod_refinements));
    }
  btor_msg_exp (btor, "");

  btor_msg_exp (btor, "linear constraint equations: %d",
    btor->stats.linear_equations);
  btor_msg_exp (btor, "gaussian elimination in linear equations: %d",
    btor->stats.gaussian_eliminations);
  btor_msg_exp (btor, "eliminated sliced variables: %d",
    btor->stats.eliminated_slices);
  btor_msg_exp (btor, "extracted skeleton constraints: %d",
  ...
  btor_msg_exp (btor, "read over write propagations during construction: %d",
		     btor->stats.read_props_construct);
  btor_msg_exp (btor, "synthesis assignment inconsistencies: %d",
...
#ifndef BTOR_DO_NOT_ELIMINATE_SLICES
  btor_msg_exp (btor, "%.2f seconds in slicing during rewriting (%.0f%%)",
    btor->time.slicing, percent (btor->time.slicing, btor->time.rewrite));
#endif
                ...
                BtorNode * exp, BtorPtrHashTable * backannotation)
{
  ...
  while (!BTOR_EMPTY_STACK (exp_stack))
      ...
      if (cur->synth_mark == 0)
	{
	  ...
	  else if (BTOR_IS_WRITE_NODE (cur) ||
...
propagated_upwards (Btor * btor, BtorNode * exp)
{
  ...
  if (BTOR_IS_WRITE_NODE (exp) && exp->e[0] == parent)
    return 1;
...
add_lemma (Btor * btor, BtorNode * array, BtorNode * acc1, BtorNode * acc2)
{
  ...
  for (acc = acc1; acc; acc = acc == acc1 ? acc2 : 0)
      ...
      for (cur = array; cur && cur != acc; cur = cur->parent)
	{
	  ...
	  if ((!prev && propagated_upwards (btor, cur)) ||
	      (prev && !(propagated_upwards (btor, prev)
	      ...
	      else if (BTOR_IS_ARRAY_COND_NODE (cur))
		{
		  ...
		  if (!BTOR_IS_BV_CONST_NODE (BTOR_REAL_ADDR_NODE (cond)))
		    {
		      assignment = btor_get_assignment_aig (amgr,
				     BTOR_REAL_ADDR_NODE (cond)->av-> aigs[0]);
		      assert (assignment == 1 || assignment == -1);
		      if (BTOR_IS_INVERTED_NODE (cond))
			assignment = -assignment;
  ...
  encode_lemma (btor, writes, aeqs, aconds_sel1, aconds_sel2,
		...
		BTOR_GET_VALUE_ACC_NODE (acc1), BTOR_GET_VALUE_ACC_NODE (acc2));
 ...
 */
static int
lazy_synthesize_and_encode_acc_exp (Btor * btor, BtorNode * acc,
				    int force_update)
{
  ...
  while (unsynthesized_constraints->count > 0)
      ...
      if (btor->rewrite_level > 2)
	{
	  ...
	  if (real_cur->kind == BTOR_BEQ_NODE)
		...
		{
	  assert (BTOR_REAL_ADDR_NODE (other->e[0])->kind != BTOR_BV_CONST_NODE);
	  assert (BTOR_REAL_ADDR_NODE (other->e[1])->kind != BTOR_BV_CONST_NODE);
		}
	    }
	}
#endif
      ...
      else
	{
	  ...
	  btor_remove_from_ptr_hash_table (unsynthesized_constraints,
					   cur, 0, 0);
	  btor_release_exp (btor, cur);
	}
    }
...
BTOR_SPLIT_SLICES_RESTART:
  ...
  if (btor->verbosity)
    btor_msg_exp (btor, 
      "sliced %d variables in %1.f seconds",
      count, delta);
}
...
process_skeleton (Btor * btor)
{
  ...
  if (btor->verbosity)
    btor_msg_exp (btor, "found %u skeleton literals in %d constraints", 
                  ids->count, count);
