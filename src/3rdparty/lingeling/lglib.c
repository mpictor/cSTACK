...
struct LGL {
  ...
  struct { int res; char chk, init; } picosat;
#endif
};
...
#ifdef NLGLPICOSAT
#if 0
  if (lgl->clone) lglchkclone (lgl->clone);
  else lgl->clone = lglclone (lgl);
#else
...
static void lglpicosatchkunsat (LGL * lgl) {
  ...
  for (p = lgl->eassume.start; p < lgl->eassume.top; p++)
    picosat_assume (lglimport (lgl, *p));
  res = picosat_sat (-1);
  assert (res == 20);
...
#else
    assert (act >= NOTALIT);
    act = NOTALIT + (lgl->stats->confs + (act - NOTALIT))/2;
#endif
    LGLCHKACT (act);
    *ap = act;
  }
...
#else
static int lgldecidephase (LGL * lgl, int lit) {
  int res = abs (lit), pos, neg, ok;
  if (lgldefphase (lgl, res) <= 0) res = -res;
  lgldassume (lgl, -res);
  assert (!lgl->flipping), lgl->flipping = 1;
  ok = lglbcpsearch (lgl);
  assert (lgl->flipping), lgl->flipping = 0;
  if (!ok) return 0;
  neg = lglcntstk (&lgl->trail);
  lglbacktrack (lgl, lgl->level-1);
  lgldassume (lgl, res);
  if (!lglbcpsearch (lgl)) return 0;
  pos = lglcntstk (&lgl->trail);
  if (pos >= neg) return 0;
  lglbacktrack (lgl, lgl->level-1);
  return -res;
}
...
static int lgltopsimp (LGL * lgl, int forcesimp) {
  ...
  if (lglterminate (lgl)) return 0;
  if (!lglpreprocessing (lgl, forcesimp)) return 20;
  if (lglfailedass (lgl)) return 20;
  if (!lgl->qscheduling) {
...
void lglrelease (LGL * lgl) {
  ...
  if (lgl->tid < 0 && lgl->picosat.init) {
    if (lgl->opts->verbose.val > 0 &&
        lgl->opts->check.val > 0) picosat_stats ();
    picosat_reset ();
  }
#endif
#endif
  ...
  if ((dealloc = lgl->mem->dealloc)) {
     ...
     dealloc (memstate, lgl->stats, sizeof *lgl->stats);
     dealloc (memstate, lgl->times, sizeof *lgl->times);
     dealloc (memstate, lgl->opts, sizeof *lgl->opts);
